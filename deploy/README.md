...
Публикация сайта отменена. Созданная сборка и отдельная конфигурация Nginx удалены с production. DNS и TLS не создавались. Файлы в этом каталоге оставлены только как неактивные шаблоны для возможного будущего развёртывания.
Исходники: https://github.com/AndreyGulevich/guitar-pro-converter

Ниже описан возможный отдельный статический сайт Nginx на сервере eva-chat.ru, а не действующая установка. Файлы Eva Chat и papalam не используются и не заменяются. Обработка музыки остаётся в браузере.
